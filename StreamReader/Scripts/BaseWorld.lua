...
function BaseWorld.sv_e_onChatCommand( self, params )
	...
	elseif params[1] == "/foodkit" then
		...
		sm.container.beginTransaction()
		sm.container.collect( container, obj_consumable_longsandwich, sm.noise.randomRange(0,1) )
	...
	elseif params[1] == "/seedkit" then
		...
		sm.container.endTransaction()
	elseif params[1] == "/memekit" then
		local chest = sm.shape.createPart( obj_container_chest, params.player.character.worldPosition + sm.vec3.new( 0, 0, 3.5 ), sm.quat.identity() )
		...
		for i, unit in ipairs( units ) do
			if InSameWorld( self.world, unit ) then
				if unit ~= nil then
					...
					if distance < 70 then
						sm.physics.explode( unit:getCharacter().worldPosition + sm.vec3.new(0,0,0.05) , 100, 50, 50, 1500, "RedTapeBot - ExplosivesHit" ) -- potentialluy ignore character?

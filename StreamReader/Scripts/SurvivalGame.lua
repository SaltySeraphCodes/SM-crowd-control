...
function SurvivalGame.server_onFixedUpdate( self, timeStep )
	...
	if g_streamReader then
		--print(g_streamReader.initialized)
...
function SurvivalGame.sv_teleport( self, params )
	local cellX = params[1]
	local cellY = params[2]
...
function SurvivalGame.cl_shield(self,params) -- Shield character Depreciated because Effects only work in survivalPlayer...
	--self.shieldFX = sm.effect.createEffect("Loot - GlowItem",params.character)
	local effectName = "Loot - GlowItem"
	sm.effect.playEffect( effectName, params.position, sm.vec3.zero(), sm.quat.identity(), sm.vec3.one(), { ["Color"] = sm.color.new(225,225,225) } )
